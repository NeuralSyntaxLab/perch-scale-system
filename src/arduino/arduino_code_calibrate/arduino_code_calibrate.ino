...
myScale.getWeight() - Returns the y of y = mx + b using the current weight on scale, the cal factor, and the offset. This function uses the average of 8 samples (by default), the known Zero offset and Calibration factor to caluculate the weight measured upon calling the function. In order to increase the accuracy, you can modify the default numer of samples by writing - myScale.getWeight(false, samplesToTake) where samplesToTake is the changed variable.
***************************************************************************/
